...
<head>
    ...
    <script type="text/javascript" src="/js/jquery-1.6.4.js"></script>
      ...
      <script type="text/javascript"    src="http://localhost:8088/sso/token/122?callback=function123">
          function123(date)/*这是跨域请求8088域之后获取的js脚本（类似ajax请求之后的的{id:1,name:jack}）  */
...
<div class="w" id="logo">
    <div>
    	<a href="http://localhost:8082">
    		<img src="/images/taotao-logo.gif" alt="宜立方商城" width="170" height="60"/>
    	</a> <b></b>
    </div>
</div>
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/sso/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/sso/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
